...
> 이 문서 하나만 보면 됨. 면접 직전 복습용.<br>
- 6/17일 에이블리 면접 대비 최초 생성
...
## 13. 순수 자바(Java) 기반 객체지향 / TDD 문제 유형

프레임워크(Spring) 없이 순수 자바 로직만으로 객체의 책임 할당, 일급 컬렉션, 원시값 포장, 테스트하기 어려운 로직의 분리 등을 묻는 기초적이면서도 중요한 유형입니다.

### 1. 주차장 구현 (Parking Lot)
> **요구사항**
> 1. 주차장(ParkingLot)은 최대 N대의 차량을 수용할 수 있습니다.
> 2. 차량(Vehicle)이 들어오면 비어있는 공간을 확인하고 주차권(Ticket)을 발급합니다. 만약 만차라면 `IllegalArgumentException`을 발생시킵니다.
> 3. 차량이 나갈 때는 주차권을 제시하며, 머문 시간에 따라 요금(시간당 1,000원)을 계산해 반환합니다.
> - **핵심 역량**: 객체 상태 관리, Map/컬렉션 활용, 시간(Time) 객체 다루기.

### 2. 로또 발매기 (Lotto)
> **요구사항**
> 1. 구입 금액을 입력하면 1장당 1,000원으로 계산해 로또를 무작위로 발급합니다. (로또 번호는 1~45 사이 중복 없는 6개 숫자)
> 2. 당첨 번호(6개)와 보너스 번호(1개)를 입력받습니다.
> 3. 발행된 로또들과 당첨 번호를 비교하여 당첨 통계(3개 일치, 4개 일치 등)와 총 수익률을 계산합니다.
> - **핵심 역량**: 도메인 규칙 캡슐화(원시값 포장, 일급 컬렉션), 불변 객체 유지, `Random` 값 생성 로직의 테스트 격리(Testability).

## 14. 실전 SQL 쿼리 (PostgreSQL 기반)

백엔드 라이브 코딩 테스트에서 도메인 로직 구현만큼이나 단골로 출제되는 것이 데이터 추출 및 가공(SQL) 능력입니다. 특히 PostgreSQL 환경에서 자주 출제되는 고급 쿼리 대표 유형입니다.

### 1. 윈도우 함수 (Window Function) 활용
> **요구사항**: 일자별 매출 데이터가 주어졌을 때, 일일 매출과 함께 **최근 7일간의 이동 평균(Moving Average)** 과 **누적 매출**을 구하는 쿼리를 작성하세요.
> - **핵심 역량**: `SUM() OVER(...)`, `AVG() OVER(...)`, `ROWS BETWEEN` 등 윈도우 함수의 작동 원리 이해.

### 2. 계층형 질의 (Recursive CTE)
> **요구사항**: 쇼핑몰 카테고리가 트리 구조(`id`, `name`, `parent_id`)로 되어 있습니다. 특정 하위 카테고리 ID가 주어졌을 때, 최상위 대분류부터 해당 카테고리까지의 **경로(Path, 예: 의류 > 상의 > 티셔츠)** 를 문자열로 결합하여 출력하세요.
> - **핵심 역량**: `WITH RECURSIVE` 구문을 활용한 재귀적 데이터 탐색.

### 3. 다중 조인 및 복잡한 집계 (Group By & Having)
> **요구사항**: 유저(`users`), 주문(`orders`), 주문상품(`order_items`), 상품(`products`) 테이블을 조인하여, 최근 30일 내에 총 결제 금액이 100만 원 이상인 유저들의 목록과, 해당 유저들이 가장 많이 구매한 '주 카테고리' 1개를 추출하세요.
> - **핵심 역량**: 다중 `JOIN` 처리, 인라인 뷰(서브쿼리) 활용, 올바른 그룹화 조건 설정.
